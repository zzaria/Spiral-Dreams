...
func _process(delta):
	...
	cam.zoom = (
		Vector2.ONE
		* 2.0 ** zoomLevel
		* min(viewSize.x / Global.VIEWPORT_SIZE.x, viewSize.y / Global.VIEWPORT_SIZE.y)
	)
...
@rpc("any_peer", "call_local")
func doAbilitiesServer(mousePos, useSlots, toggleSlots):
	if id != multiplayer.get_remote_sender_id() || !is_multiplayer_authority():
		return
...
@rpc("any_peer", "call_local")
func doMovementServer(_delta, inputDir):
	if id != multiplayer.get_remote_sender_id() || !is_multiplayer_authority():
		return
...
func _input(event):
	...
	if event.is_action_pressed("inventory"):
		get_node("CanvasLayer/InventoryScreen").visible = !(
			get_node("CanvasLayer/InventoryScreen").visible
		)
		return
	...
	for i in range(inventoryRows):
		if event.is_action_pressed("hotbar" + str(i + 1)):
			hotbarRow = i
			return
...
@rpc("any_peer", "call_local")
func _on_respawn_pressed_server():
	if id != multiplayer.get_remote_sender_id() || !is_multiplayer_authority():
		return
	respawn.emit(self)
...
@rpc("authority", "call_local", "reliable")
func showteamrequest(id, name):
	teamRequests += [[id, name]]
	get_node("CanvasLayer/TeamRequests").show()
	get_node("CanvasLayer/TeamRequests/MarginContainer/VBoxContainer/Label").text = (
		"Add to team" + name + " " + str(id) + "?"
	)
...
func setAllowedActions(team, respawn, levelStarted):
	...
	get_node("CanvasLayer/InventoryScreen/VBoxContainer/HostActions/Button").text = (
		"End game" if levelStarted else "Start game"
	)
